...
class chat_commands(object):
    def __init__(self, channel_type, channel_id, ts, user_id, current_text):
    ...
    def cacheinfo(self):
        conversations = ConversationCache(self.channel_type,self.ts).get_msg()
        cache_info = {
    ...
    def clear(self):
        ConversationCache(self.channel_type,self.ts).clear_msg()
        return '对话已清空'
